...
export const useWineSetItem = ({
    ...
    const defaultImageUrl = item.wine.image?.smallUrl || item.wine.image?.mediumUrl || item.wine.image?.originalUrl ||item.wine.defaultImage?.smallUrl || item.wine.defaultImage?.mediumUrl || item.wine.defaultImage?.originalUrl || '';

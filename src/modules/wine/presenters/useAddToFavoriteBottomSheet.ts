...
export const useAddToFavoriteBottomSheet = (wineId?: number, onUpdateIsSaved?: (isSaved: boolean) => void) => {
    ...
    const onSave = useCallback(async () => {
        ...
        try {
            ...
            const addPromises = listsToAdd.map(listId => favoriteWineService.addWineToList(listId, { wineId }));

            const removePromises = listsToRemove.map(listId => favoriteWineService.removeWineFromList(listId, wineId));
    ...
    return { favoriteData, isVisible, onItemPress, onClose, onOpen, onSave, isLoading, isSaving };
};
